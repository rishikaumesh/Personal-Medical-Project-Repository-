...
def evaluate_model(model, dataloader, device, output_dir="plots", class_names=["Normal", "Pneumonia"], model_name="model"):
    """
    ...
    Args:
        ...
        output_dir (str): Base directory to save the evaluation plots (separate for each model).
        class_names (list): Names of the classes for the confusion matrix.
        model_name (str): Name of the model for dynamically saving results.

    Returns:
        dict: Dictionary containing evaluation metrics (precision, recall, F1 score, accuracy).
    """
    # Create output directory for the specific model
    ...
    roc_curve_path = os.path.join(model_output_dir, "roc_curve.png")
